#' Diccionario cobertura vegetal
#'
#' @param filename Path del archivo .shp
#'
#' @return sf object
#' @export
#'
...
diccionario_cv <- function(filename) {
  ...
  #eliminar subguiones de lso nombres de las columnas
  cv_names <- names(shp)
  colnames(shp) <- gsub("_", "", cv_names)
  #mostrar el encabesado de las columnas
  print(head(shp))
  message(crayon::red(toupper(gsub(".shp",
                                   "",
                                   stringr.plus::str_extract_after(string = filename,
                                                                   pattern = '/',
                                                                   which = "last")))))
  # ingresar codigo de las zona de reglamentacion
  zre <- readline( prompt = "Seleccionar la variable ZRE:")
  if(!grepl("[a-z][0-9]", zre)){
    zre <- NA_real_
  }
  #' seleccionar el nombre de la columna que contiene la informacion
  ...
  out <- shp %>%
    dplyr::select(dplyr::all_of(vars1),
                  geometry)
  #' renombrar las columnas
  colnames(out) <- names_vars1
  # calcular el area de los poligonos
  polygon_area <- sf::st_area(out)
  #
  vars_2 <- names(out)

  output <- out %>%
    dplyr::mutate(zre = paste0("ZRE", zre) %>% toupper(),
    ...
    dplyr::select(dplyr::all_of(vars_2), zre, distrito, documento,
                  doc_reglam, fuente, area_m2, area_ha) %>%
    dplyr::relocate(geometry, .after = dplyr::last_col())

  # validar la informacion necesaria, con base en el diccionario de datos
  nombres_cv <- c("tipo_cober", "cobert_veg", "zre", "distrito", "documento",
  ...
  if(length(names(output)) == 10){
    ...
    message(crayon::green(paste("Es necesario añadir informacion!!!",
